...
public class MemberController extends HttpServlet {
	...
	private void join(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String persistToken = UUID.randomUUID().toString(); //난수 발생(unique id)
	...
	private void logout(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//	   session scope를 가진 객체
		//	   session 자체를 지우거나 쿠키를 삭제하면 인증정보가 아닌 이외의 필요한 정보도 같이 삭제된다.
	...
	private void login(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//1. DataBase 또는 Service 단에서 문제가 생겨서 예외가 발생 => Service에서 처리
		member = memberService.memberAuthenticate(userId, password);
		
		//2. 사용자가 잘못된 아이디와 비밀번호를 입력한 경우 => Controller에서 처리
