...
public class MemberDao {
	...
	//userId로 ' or 1=1 or user_id = ' 값을 전달받으면 모읃 회원의 비밀번호가 수정
	//SQL Injection 공격
	//악의적인 SQL구문을 주입해서 상대방의 DataBase를 공격하는 기법
	
	//SQL Injection 공격을 막기 위해 PreparedStatement 사용
	//1. 인스턴스를 생성할 때 쿼리 템플릿을 미리 등록
	//	 생성시 등록된 쿼리 템플릿의 구조가 변경되는 것을 방지
	//	 코드에서 작성한 SQL구문의 예약어들을 템플릿으로 저장, 사용되지 않은 예약어들이 사용자로부터 입력되면 실행 중단 
	//2. 문자열에 대해서 자동으로 이스케이프 처리
	...
	public void updateMemberGrade(String userId, int bookCnt, Connection conn) {
		CallableStatement cstm = null;
		String query = "{call sp_update_grade(?, ?)}";
		
		try {
			cstm = conn.prepareCall(query);
			cstm.setString(1, userId);
			cstm.setInt(2, bookCnt);
			cstm.executeUpdate();
		} catch (Exception e) {
			throw new DataAccessException(e);
		} finally {
